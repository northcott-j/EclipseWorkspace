...
public final class StrictCoinGameModel implements CoinGameModel {
  ...
  *
   */
    ...
    @Override
    public void addPlayers(int playerIndex, String... names) {
        if (playerIndex < 0 || playerIndex > this.players.size() - 1) {
        ...
        if (playerIndex == 0) {
            ...
            for (int i = 0; i < len; i += 1) {
                this.validPlayer(lNames.get(i));
                if (i != len - 1 && lNames.subList(i + 1, len).contains(lNames.get(i))) {
                    throw new IllegalArgumentException(lNames.get(i)+ " is already taken");
                }
            }
            this.players.addAll(playerIndex, Arrays.asList(names));
        }
    }
    ...
    private void nextTurn() {
        this.priorPlayer = this.players.remove(0);
